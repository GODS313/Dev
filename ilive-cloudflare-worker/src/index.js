...
export default {
  async fetch(request, env) {
    if (request.method === "GET") {
      return Response.json({ ok: true, service: "iLive Telegram Relay" });
    }
    ...
    const valid = await verifyHmac(raw, request.headers.get("X-iLive-Signature"), env.RELAY_SECRET);
    ...
    if (!env.TG_BOT_TOKEN || !env.TG_CHAT_ID) {
      return Response.json({ ok: false, error: "Telegram secrets are not configured" }, { status: 500 });
    }
    ...
    try {
      ...
      return new Response(result, { status: tg.status, headers: { "content-type": "application/json; charset=utf-8" } });
    } catch (e) {
