...
describe('commerce: store, orders, pricing, stock', () => {
  ...
  before(async () => {
    ...
    const p = await h.app.inject({ method: 'POST', url: `/api/v1/workspaces/${m.workspaceId}/products`, headers: m.auth, payload: { name: 'Green tea', priceMinor: 1250, stock: 3 } });
    variantId = p.json().variants[0].id;
    const s = await h.app.inject({ method: 'POST', url: `/api/v1/workspaces/${m.workspaceId}/products`, headers: m.auth, payload: { name: 'Tasting', kind: 'service', priceMinor: 5000 } });
  ...
  it('rejects unknown fields on workspace update (mass assignment)', async () => {
    const res = await h.app.inject({ method: 'PATCH', url: `/api/v1/workspaces/${m.workspaceId}`, headers: m.auth, payload: { owner_user_id: '00000000-0000-0000-0000-000000000000' } });
  ...
  it('customer order uses server-side prices, is idempotent and decrements stock', async () => {
    ...
    const place = () =>
      h.app.inject({
        ...
        headers: c.auth,
        payload: { items: [{ variantId, quantity: 2 }, { variantId: serviceVariant, quantity: 1 }], idempotencyKey: 'order-key-0001', priceMinor: 1 },
      });
  ...
  it('refuses orders exceeding stock and variants from other stores', async () => {
    ...
    const over = await h.app.inject({ method: 'POST', url: `/api/v1/store/${m.slug}/orders`, headers: c.auth, payload: { items: [{ variantId, quantity: 5 }], idempotencyKey: 'order-key-0002' } });
    assert.equal(over.statusCode, 409);
    const other = await loginWithTrial(h, 4004, 'Other');
    const op = await h.app.inject({ method: 'POST', url: `/api/v1/workspaces/${other.workspaceId}/products`, headers: other.auth, payload: { name: 'Foreign', priceMinor: 1 } });
  ...
  it('concurrent orders cannot oversell the last unit', async () => {
    ...
    const results = await Promise.all(
      buyers.map((b, i) =>
        h.app.inject({ method: 'POST', url: `/api/v1/store/${m.slug}/orders`, headers: b.auth, payload: { items: [{ variantId, quantity: 1 }], idempotencyKey: `race-key-000${i}` } }),
      ),
    );
  ...
  it('merchant sees orders and customers; status machine rejects invalid transitions', async () => {
    ...
    assert.ok(order);
    const bad = await h.app.inject({ method: 'POST', url: `/api/v1/workspaces/${m.workspaceId}/orders/${order.id}/status`, headers: m.auth, payload: { status: 'refunded' } });
    assert.equal(bad.statusCode, 409);
    for (const status of ['confirmed', 'paid', 'fulfilled']) {
      const ok = await h.app.inject({ method: 'POST', url: `/api/v1/workspaces/${m.workspaceId}/orders/${order.id}/status`, headers: m.auth, payload: { status } });
  ...
  it('cancelling an order returns stock', async () => {
    ...
    await h.app.inject({ method: 'PATCH', url: `/api/v1/workspaces/${m.workspaceId}/products/${(await h.db.system.query('SELECT product_id FROM product_variants WHERE id=$1', [variantId])).rows[0].product_id}`, headers: m.auth, payload: { stock: 2 } });
    const r = await h.app.inject({ method: 'POST', url: `/api/v1/store/${m.slug}/orders`, headers: c.auth, payload: { items: [{ variantId, quantity: 2 }], idempotencyKey: 'cancel-key-0001' } });
    assert.equal(r.statusCode, 200, r.body);
    await h.app.inject({ method: 'POST', url: `/api/v1/workspaces/${m.workspaceId}/orders/${r.json().order.id}/status`, headers: m.auth, payload: { status: 'cancelled' } });
    const stock = await h.db.system.query('SELECT stock FROM product_variants WHERE id = $1', [variantId]);
